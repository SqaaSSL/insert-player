...
Release status on 2026-08-22: the full Pages app, Clerk Production, dedicated Google OAuth, and live Stripe configuration are active. Production API `0.16.0` is the last deployed backend while the `0.17.0` durable-job release is promoted from `main`. QA already runs `0.17.0` with migrations through `0019`, the Workflow, and the Container; its deployed smoke and two real reconnecting Retry jobs pass.
...
- D1 migrations: production currently has `0001` through `0017`; the release workflow applies `0018`/`0019` before deploying `0.17.0`. Those migrations add durable generation/retry jobs, progress events, idempotent provider-request records, and exact retry targets while preserving existing rows.
...
- Still missing before calling the beta production-ready: promote/smoke Worker `0.17.0`, use two fresh Clerk Production Google users for authenticated launch smoke, complete one live Stripe purchase/webhook, prove one real Turnstile token and its rejected replay, verify support-address delivery/reply, run real-phone QA, and complete physical two-device validation. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA.
