...
- D1 migrations: production and QA have `0001` through `0023`. Migrations `0018`/`0019` add durable generation/retry jobs, progress events, idempotent provider-request records, and exact retry targets. Migrations `0020`-`0022` add official Arcade state/prompt provenance and provider-capacity windows. Migration `0023` backfills immutable artifact runs/checkpoints and resumable job lineage while preserving every existing source/sprite version and provider response.
...
- Still missing before calling the beta production-ready: replace the expired GitHub Cloudflare token with a durable account token that includes Cache Purge and rerun both deployment workflows; add the Clerk Development backend key to GitHub and pass the disposable authenticated sandbox smoke; finish and activate the QA-approved Gemini-only Arcade roster; complete one live Stripe purchase/webhook; verify support-address delivery/reply; run real-phone QA; and complete physical two-device validation. The real production Turnstile success/replay check is complete. Production authenticated smoke Action `32703046701` passed with two dedicated Google OAuth users, distinct Clerk sessions, D1/R2 writes, owner privacy, publish/unpublish, public-only cloning, cleanup, and session revocation. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA.
