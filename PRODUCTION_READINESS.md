...
- Cloudflare D1: stores Clerk users, fighter metadata, Crew grants, HMAC-pseudonymized referral eligibility, Rookie referral entitlements, sprite records, content-hashed source/sprite version archives, credit ledger, checkout sessions, generation charges, provider sessions, artifact runs/checkpoints, atomic monthly provider-spend accounting, permanent per-call cost events, atomic rate-limit counters, community moderation reports, matches, and leaderboard rows. Aggregate spend is observable but does not globally disable paid generation.
...
- Clerk: isolated Insert Player frontend sign-in/up, Organizations as Crews, Worker JWT verification, Backend API invitations/verified-social referral checks, and signed user/Organization lifecycle webhooks.
...
- Create a dedicated Insert Player Clerk application. Production Organizations are enabled by the canonical deployment only after the Crew-capable Worker and exact lifecycle webhook are verified; personal accounts remain available so the guided debut can precede Crew creation. The application may use the same company/team account as ShellBot, but its consent/sign-in surfaces, keys, users, Organizations, domains, and branding must be Insert Player-specific so a player never lands on a ShellBot permission screen.
...
- The Development endpoint may subscribe to the complete event catalog while QA explores Google, Apple, and test-email behavior. The Worker signature-verifies every delivery, acknowledges unsupported event types with `200`, and does not persist their payloads. Production subscribes only to `user.created`, `user.updated`, `user.deleted`, `organizationInvitation.accepted`, `organizationMembership.deleted`, and `organization.deleted`: these are the events the lifecycle and Crew handlers consume. The canonical production workflow verifies the exact endpoint and signing secret, applies this event allowlist, and enables Organizations only afterward. Unused email events may carry verification material, session events add avoidable volume, and an all-events subscription would opt into future payload types without review. Store each instance's distinct signing secret as `CLERK_WEBHOOK_SIGNING_SECRET` and its matching Backend API key as the Worker secret `CLERK_SECRET_KEY`.
...
4. Open `/legal`, `/privacy`, `/terms`, and `/refunds`. Confirm legal version `2026-09-15.1`, operator/address/registry details, the visible Cancellations label, and AI-generated labels. Confirm generation consent says credits are consumed when external AI begins, are not automatically restored after provider failure, and preserve mandatory remedies. It must also authorize only processing needed to create and save the fighter, reject any unrelated publication/reuse licence, say the original is never published, and exclude sale, advertising use, and Insert Player model training. Confirm the visible access choices are Crew and Community only: Crew exposes authenticated clean playable derivatives to the selected Organization, while Community requires a separate confirmation. Both exclude account name/email, original, Clerk profile photo, RAW intermediates, private hashes, and history. Every generation/retry/upgrade and credit checkout must remain disabled until its current versioned consent is checked.
...
8a. Complete the Aura onboarding happy path: guided trial, included Rookie creation, automatic Rookie debut, Crew creation/selection, Crew sharing, and one invitation. Accept from a genuinely new verified Google, Apple, or Microsoft account, create its included Rookie, and complete its Aura debut. Confirm the inviter receives exactly one non-transferable Rookie pass only after the debut, never at send/accept, and that the invitee's Crew roster can play the shared fighter without receiving original/RAW/account fields. Confirm the lifetime reward cap is three and invitation velocity is five active invitations per day.
