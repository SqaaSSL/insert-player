...
- Cloudflare Container: runs the existing TypeScript image/generation pipeline for the Workflow. The browser starts and observes jobs; server-side code alone commits or refunds the reservation.
...
Release status on 2026-08-23: the full Pages app, Clerk Production, dedicated Google OAuth, live Stripe configuration, and production API `0.17.0` are active. Production and QA both run migrations through `0019` with isolated Workflows and Containers. Public production smoke, Pages smoke, and live readiness pass; QA additionally passed two real reconnecting Retry jobs.

- Worker diagnostics URL: `https://ai-street-fighter-api.shellbot.workers.dev`; public API: `https://api.insertplayer.ai`.
- Worker `0.17.0`, version `6bded145-7320-4c8e-ac0f-22199e059367`, deployed with the production proxy/auth/billing/storage code, server-owned `FIGHTER_GENERATION` Workflow, EU `IMAGE_PROCESSOR` Container, idempotent provider-response cache, reconnecting job progress, observability, operation-specific credit reservations, durable provider cost events, exact apex+`www` CORS/Turnstile allowlists, fixed `clerk.insertplayer.ai` issuer, dedicated Insert Player Stripe account pin, Clerk account-lifecycle handling, HMAC-pseudonymized anonymous identifiers, versioned generation/checkout consent, streaming upload/provider-request limits, atomic D1 billing/rate-limit writes, transactionally versioned direct uploads/community copies, authenticated report intake, admin-only audited moderation, account-pinned Stripe catalog checks, automatic tax and reusable per-user Stripe Customers, bounded/idempotently retried Stripe requests, out-of-order-safe refund/dispute wallet reconciliation, purpose/tier route and GA Gemini model allowlists, per-session cost ceilings, a `$500/month` global provider reserve, hardened result redirects/downloads, non-PII Stripe event storage, operational retention, and the isolated D1/R2 bindings. Private fighter manifests expose current and archived source/processed/RAW hashes so clients upload only missing blobs and can promote known versions without re-uploading bytes. AI arena sessions are additionally limited to one Flash call / 10¢, five per free signed-in account/day, and one client-cached result per arena theme.
- Pages project: `insert-player`; Pages URL `https://insert-player.pages.dev/`. `insertplayer.ai` and `www.insertplayer.ai` are active custom domains with certificates and serve the full product app. The credential-free prelaunch build remains a checked emergency/holding artifact, not the current public surface.
...
- Still missing before calling the beta production-ready: add Cloudflare `Containers: Write` to the GitHub production token and rerun the protected Action, use two fresh Clerk Production Google users for authenticated launch smoke, complete one live Stripe purchase/webhook, prove one real Turnstile token and its rejected replay, verify support-address delivery/reply, run real-phone QA, and complete physical two-device validation. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA; the production durable release and public smoke/readiness are live.
...
16. Force a durable generation failure and confirm the Workflow refunds the reservation server-side without requiring the browser to call a completion endpoint. Retry the identical start request concurrently and confirm only one job and one reservation survive.
...
- Promote and smoke Worker `0.17.0` with migrations `0018`/`0019`, Workflow, and Container, then validate one real production Turnstile token and reject its replay.
- Real Rookie, Contender, Champion, failed-upgrade refund, Retry, cloud-history, and durable navigation/reconnect paths have passed in authenticated sandbox. Carry that evidence into the final launch record; do not regenerate solely to repeat provider spend.
