...
- Still missing before calling the beta production-ready: replace the working temporary Cloudflare OAuth credential with a long-lived account token that includes Cache Purge, finish and activate the QA-approved Gemini-only Arcade roster, use two fresh Clerk Production Google users for authenticated launch smoke, complete one live Stripe purchase/webhook, prove one real Turnstile token and its rejected replay, verify support-address delivery/reply, run real-phone QA, and complete physical two-device validation. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA; the protected production and develop Actions, durable release, and public smoke/readiness are green.
...
- Validate one real production Turnstile token and reject replay of that exact token; Worker `0.17.0`, migrations `0018`/`0019`, Workflow, Container, public smoke, Pages smoke, and live readiness are already promoted and green.
