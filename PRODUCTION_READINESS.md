...
- Cloudflare Workflow: owns authenticated new-fighter generation, tier upgrades, animation Retry, and canonical Pro source Retry. D1-backed progress and idempotent provider-request records let the UI reconnect without duplicating a charge or provider write.
...
- Cloudflare D1: stores Clerk users, fighter metadata, sprite records, content-hashed source/sprite version archives, credit ledger, checkout sessions, generation charges, provider sessions, atomic monthly provider-spend accounting, permanent per-call cost events, atomic rate-limit counters, community moderation reports, matches, and leaderboard rows. Aggregate spend is observable but does not globally disable paid generation.
...
Release status on 2026-08-24: the full Pages app, Clerk Production, dedicated Google OAuth, live Stripe configuration, and production API `0.18.0` are active. Production and QA both run migrations through `0019` with isolated Workflows and Containers. The release removes the legacy `$500/month` and `$9/10-minute` global cutoffs while retaining paid credits, Turnstile, per-user/IP limits, approved route/model boundaries, per-session call/cost ceilings, atomic monthly accounting, and permanent per-call cost events. Protected production Action `32705119211` deployed main commit `c43068c`; protected development Action `32705341062` deployed develop commit `0a3f704`. Both passed the production gate, Worker migration/deploy/smoke, Pages deploy, and canonical frontend smoke. Runtime source, the compiled processor bundle, and the Container build fail closed on unapproved BFL/FLUX/Klein endpoints or imports; official Arcade generation remains Gemini-only. The refreshed temporary Wrangler OAuth token in GitHub makes team deploys operational today, but it must still be replaced with a durable least-privilege account token that includes Cache Purge.

- Worker diagnostics URL: `https://ai-street-fighter-api.shellbot.workers.dev`; public API: `https://api.insertplayer.ai`.
- The approved `0.18.0` code-bearing release at main commit `c43068c` was deployed and live-validated as Worker version `c474a651-2140-4862-ae2d-a2dd3ab4c9b6`. Later documentation-only deploys may receive a new Cloudflare version id without changing that approved runtime bundle. It preserves the production proxy/auth/billing/storage code, server-owned `FIGHTER_GENERATION` Workflow, EU `IMAGE_PROCESSOR` Container, idempotent provider-response cache, reconnecting job progress, operation-specific credit reservations, first-provider charge commitment with no automatic restoration after billable inference, exact apex+`www` CORS/Turnstile allowlists, fixed `clerk.insertplayer.ai` issuer, dedicated Insert Player Stripe account pin, Clerk account lifecycle, HMAC-pseudonymized anonymous identifiers, versioned consent, streaming request limits, transactional asset versioning, audited moderation, Stripe reconciliation, purpose/tier route and GA Gemini model allowlists, conservative per-session call/cost ceilings, atomic unbounded monthly accounting, and permanent per-call provider cost events. AI arena sessions remain limited to one Flash call / 10¢, five per free signed-in account/day, and one client-cached result per arena theme.
- Pages project: `insert-player`; Pages URL `https://insert-player.pages.dev/`. Code-bearing production deployment preview `https://9ed588c6.insert-player.pages.dev` served main commit `c43068c`; both preview and canonical frontend smoke passed, followed by live readiness. Later documentation-only previews do not change the approved frontend bundle. `insertplayer.ai` and `www.insertplayer.ai` are active custom domains with certificates and serve the full product app. The credential-free prelaunch build remains a checked emergency/holding artifact, not the current public surface.
...
- D1 migrations: production and QA have `0001` through `0019`. Migrations `0018`/`0019` add durable generation/retry jobs, progress events, idempotent provider-request records, and exact retry targets while preserving existing rows.
...
- Isolated QA Worker: `https://insert-player-api-sandbox.shellbot.workers.dev`. The approved `0.18.0` code-bearing release at develop commit `0a3f704` was deployed and smoke-tested as version `0593dc5c-d721-4e70-b32b-73bd8a89178f`; later documentation-only deploys may assign a new version id without changing the runtime bundle. `ENVIRONMENT=sandbox`, production-origin CORS is denied, Clerk Development is configured, and dedicated test-mode Stripe billing is configured. The matching code-bearing Pages preview is `https://c1d90b72.insert-player-sandbox.pages.dev`; preview and canonical smoke both passed. The release removes the old `$50/month` service cutoff while preserving isolated monthly/event accounting and all authenticated controls. The QA Workflow is `insert-player-fighter-generation-sandbox`; the image-processor Container is isolated from production. Provider secrets are present for authenticated QA, while `ANONYMOUS_ROOKIE_ENABLED=false` blocks public provider-session minting before cost.
- Isolated QA D1: `insert-player-sandbox-db` (`f60b6e22-d262-4e46-a7d9-ca095e49d102`, EU jurisdiction, EEUR primary region), with migrations `0001` through `0019` applied
...
- Still missing before calling the beta production-ready: replace the working temporary GitHub Cloudflare OAuth token with a long-lived account token that includes Cache Purge; add the Clerk Development backend key to GitHub and pass the disposable authenticated sandbox smoke; finish and activate the QA-approved Gemini-only Arcade roster; complete one live Stripe purchase/webhook; verify support-address delivery/reply; run real-phone QA; and complete physical two-device validation. The real production Turnstile success/replay check is complete. Production authenticated smoke Action `32703046701` passed with two dedicated Google OAuth users, distinct Clerk sessions, D1/R2 writes, owner privacy, publish/unpublish, public-only cloning, cleanup, and session revocation. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA. Team deployment is operational with the temporary token; token durability remains an infrastructure handoff item rather than a current outage.
