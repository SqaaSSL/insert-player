...
- Isolated QA Worker: `https://insert-player-api-sandbox.shellbot.workers.dev`, version `0.17.0` (`44d04769-c49d-4beb-be70-65925d1081d6`), deployed from develop commit `074383e`, `ENVIRONMENT=sandbox`, production-origin CORS denied, Clerk Development configured, and dedicated test-mode Stripe billing configured. It still has the legacy `$50/month` reserve until candidate `0.18.0` deploys; the candidate removes that service cutoff while preserving isolated monthly/event accounting and all authenticated controls. The QA Workflow is `insert-player-fighter-generation-sandbox`; the image-processor Container is isolated from production. Provider secrets are present for authenticated QA, while `ANONYMOUS_ROOKIE_ENABLED=false` blocks public provider-session minting before cost.
...
- Still missing before calling the beta production-ready: replace the expired GitHub Cloudflare OAuth token with a long-lived account token that includes Cache Purge, finish and activate the QA-approved Gemini-only Arcade roster, use two fresh Clerk Production Google users for authenticated launch smoke, complete one live Stripe purchase/webhook, verify support-address delivery/reply, run real-phone QA, and complete physical two-device validation. The real production Turnstile success/replay check is complete. Real provider generation, cloud archival, durable reconnect, and the authenticated Stripe purchase/webhook path have passed in isolated QA; the last protected durable release and current public Worker/Pages smokes remain proven, while new team deploys are blocked until token replacement.
