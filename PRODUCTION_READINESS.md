...
- Create a dedicated Insert Player Clerk application with Organizations enabled. It may use the same company/team account as ShellBot, but its consent/sign-in surfaces, keys, users, Organizations, domains, and branding must be Insert Player-specific so a player never lands on a ShellBot permission screen.
