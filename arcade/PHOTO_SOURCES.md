...
The seeder asks Clerk for short-lived session tokens and refreshes them during polling. It never writes or prints those credentials. A pre-minted `ASF_ARCADE_ADMIN_JWT` remains available for short runs, but it must have at least five minutes left. Before reading or mutating the Arcade inventory, the seeder asks the deployed processor to prove its runtime contract: Gemini Pro for all three canonical sources, Gemini Flash only for the Champion pose scaffold, Gemini Pro for every visible Champion frame and visual review, and a fail-closed fallback policy. Any other provider or model aborts the run before generation. Generate one fighter as a draft, inspect all canonical views and animations, and only then rerun with `--activate` or continue to the next fighter.
