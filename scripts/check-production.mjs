...
function assertBillingUsesRequestOrigin() {
  ...
  const required = [
    ...
    'quoteGenerationPackage(tier, creationPackage)',
    'quoteOwnedPackageExpansion(env, auth.user.id, ownedFighterId!',
...
function assertClerkAuthIsWired() {
  ...
  const required = [
    ...
    "tryGame('aura')",
    "if (route !== '/') return;",
    'creationDestination(creationContext.returnTo)',
    'UserButton',
    'isLoaded',
    "'loading'",
...
function assertGithubActionsAreWired() {
  const files = {
    ...
    dependencyAlertGate: 'scripts/check-dependency-alerts.mjs',
  ...
  const required = {
    validation: [
      ...
      'node scripts/check-dependency-alerts.mjs',
    ],
    dependencyAlertGate: [
      'dependabot/alerts',
      '--paginate',
      'security_advisory?.severity',
      'evaluateDependencyAlerts',
      'semver.satisfies',
      'process.exitCode = 1',
    ],
    ci: [
