...
Production serves the full app with Clerk Production, dedicated live Stripe configuration, and Cloudflare Workflow/Container generation. QA remains the environment for paid-provider generation and test Checkout. Main `993cd72` and develop `c6589b3` have the same production-safe tree; both databases are migrated through `0024`. Production runs Worker version `e1d68b66-31fd-4d2b-b912-d835fc503e6a`, while QA runs `b35b2b17-fb0f-4f81-b2cb-5c7e18c9e789`. Public Worker, canonical Pages, live-readiness, and two-user authenticated production smoke all pass. Promotion from `main` normally runs migrations, checks, Worker/Container deploy, Worker smoke, Pages deploy, and readiness. Production Action `32741006866` and development Action `32741330991` passed their complete local gates, then stopped at the first remote Cloudflare mutation because GitHub's temporary token is invalid (`9109 Invalid access token`). Replace it with a durable token for the exact SqaaS account that includes Workers Platform Admin and zone Cache Purge access, then rerun both workflows.
