...
- The original photo and RAW fighter history stay account-restricted. The visible access choices are Crew and Community: authenticated members of the selected Crew receive only clean generated source views/playable assets, while Community requires a separate confirmation and uses the neutral author label `Player`. Neither exposes account names, emails, Clerk profile photos, Clerk/internal account ids, original uploads, RAW intermediates, private hashes, or archived history. Public media uses revocable opaque URLs that never expose the owner-scoped R2 key. A future public handle requires separate opt-in.
- Each Crew can lock one included shared home stage, enforced by the Clerk Organization id rather than by member. Only a Crew admin can choose it; all active members can use the private final 1024×576 derivative, while the raw source capture is not retained in cloud storage. The onboarding invites the players who should vote before this single immutable choice is confirmed.
