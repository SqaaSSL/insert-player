...
describe('hash-bound Aura pose registration', () => {
  it('uses each new official atlas standing reference without changing its authored poses', () => {
    for (const assets of Object.values(ADDITIONAL_AURA_BUILTIN_ASSETS)) for (const asset of assets) {
      const input = { ...atlas(asset.name), contentHash: asset.contentHash };
      const result = calibrateAuraAtlas(input, idle);
      expect(result.policy).toBe('bundled-reference-v1');
      expect(result.referenceBodyHeight).toBe(asset.referenceBodyHeight);
      expect(result.frames).toHaveLength(8);
      expect(result.frames.every(frame => frame.scale === 1 && frame.originX === 0.5 && frame.offsetX === 0 && frame.offsetY === 0 && frame.sourceFrame === undefined)).toBe(true);
      expect(result.frames[0].originY).toBe(asset.referenceRootY / asset.frameHeight);
      expect(() => calibrateAuraAtlas({ ...input, bounds: [null, ...input.bounds.slice(1)] }, idle)).toThrow('invalid reviewed Aura frame bounds');
      // Re-generated bytes and a differently shaped atlas cannot inherit this calibration.
      expect(calibrateAuraAtlas({ ...input, contentHash: 'another-atlas' }, idle).policy).toBe('shared-idle-v1');
      expect(calibrateAuraAtlas({ ...input, frameWidth: input.frameWidth * 2 }, idle).policy).toBe('shared-idle-v1');
    }
  });
