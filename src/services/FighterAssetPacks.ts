...
export type FighterModeReadinessKind = 'custom' | 'unavailable';
...
export function invalidAssetPackAnimationNames(
  ...
  for (const asset of assets) {
    ...
    const hasAuraPlayback = packId !== AURA_ASSET_PACK_ID || (
      ('pngBlob' in asset || 'url' in asset)
      && Number.isInteger(asset.frameWidth)
      && Number.isInteger(asset.frameHeight)
      && Number.isInteger(asset.frameCount)
    );
    if (hasAuraPlayback && hasUsablePlaybackMetadata(asset)) usable.add(asset.animationName);
  }
...
/**
 * Fight and Rush share the combat pack. Aura requires all six dedicated
 * performances, either as usable assets or the reviewed bundle belonging to
 * this exact official Arcade identity. Combat animations cannot qualify it.
 */
export function resolveFighterModeReadiness(
  assets: ReadonlyArray<AssetPackSprite>,
  mode: FighterGameMode,
  identity?: AuraBuiltinPerformerIdentity | null,
  ...
  if (aura.complete || builtinAuraPerformerForIdentity(identity)) {
    return {
      mode,
      kind: 'custom',
...
export function assertFighterReadyForMode(
  ...
  const readiness = resolveFighterModeReadiness(assets, mode, identity);
...
export function isAnimationNameSetReadyForMode(
  ...
  return hasPack(AURA_ASSET_PACK_ID) || builtinAuraPerformerForIdentity(identity) !== null;
}
