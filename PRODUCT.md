...
Detailed playtest events remain exportable on the device. First-party measurement also combines fixed gameplay, creation and Crew event names, bounded durations and a fixed acquisition-channel name into daily totals retained for 90 days. It introduces no visitor/session IDs, cookies, raw campaign text, referrers or account-linked analytics stream, and honors browser Do Not Track and Global Privacy Control. An admin report separates these best-effort event counts from existing stored account, generation, invite, stage and purchase outcomes. These are operational signals, not unique-person conversion cohorts or cross-device retention. See `docs/product-measurement.md` for the limits and report command.
