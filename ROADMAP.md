...
- **Insert Player has isolated production storage**: D1 `insert-player-db` (`aa9e6ba6-8a7f-4261-ac31-986e6cf44659`) and R2 `insert-player-assets` are in the EU. Production currently has migrations through `0017`; release migrations `0018`/`0019` add durable generation/retry jobs and will be applied automatically before the `0.17.0` Worker deploy. Provider spend has atomic monthly and rolling reserves plus a permanent per-call cost ledger keyed by billing operation/outcome; scheduled cleanup cannot delete that ledger, credits, fighters, versions, users, tombstones, or R2 assets. R2 remains private behind the Worker and its `temp/` lifecycle deletes after one day.
- **The full production app is live; the durable backend release is staged**: `https://insertplayer.ai` now serves the complete product shell with Clerk Production and dedicated live Stripe configuration, not the old holding build. Production API `0.16.0` remains live at `https://api.insertplayer.ai` until this `0.17.0` release reaches `main`; its health reports Clerk lifecycle, live Stripe, Turnstile, providers, D1/R2, and spending guards configured. GitHub Actions applies migrations before deploying the Workflow/Container Worker, then deploys/smokes Pages and runs live readiness.
...
Status: implemented for local and durable server-owned generation, cloud metadata, Create tier picker, Gallery tier badges/upgrades/retries, operation-specific credits, and version preservation. The remaining risk is launch evidence and production promotion, not tier pipeline design.
