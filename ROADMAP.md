...
- **Authenticated generation is now backend-owned and durable**: Create, tier upgrades, animation Retry, and canonical Pro source Retry start a signed Cloudflare Workflow job instead of depending on the page remaining alive. D1 records progress/events and idempotent provider writes, while the `IMAGE_PROCESSOR` Container runs the existing TypeScript pipeline. Reopening Create/Gallery discovers and reconnects to an active job; closing the tab or losing connectivity does not cancel generation. Charge commit/refund happens server-side. Identical concurrent starts replay one job/reservation, while a genuinely different competing authorization is refunded.
...
- **Current legal and publication boundaries are production-verified**: legal version `2026-08-22.5` exposes `/legal`, `/privacy`, `/terms`, and `/refunds`, uses the current BORME registered office and sheet, and makes the generation boundary explicit. Consent authorizes only the processing needed to create and privately save the fighter and explicitly rejects any general publication/reuse licence. The original photo is never published; Insert Player does not sell it, advertise with it, or use it for model training. A separate Publish confirmation exposes only that fighter's clean generated sources/playable assets under the neutral author label `Player`; account names, emails, Clerk profile photos, Clerk/internal account ids, originals, RAW intermediates, private hashes, and history remain private. A future public handle requires separate opt-in. Real QA first caught an email-shaped display name and then an owner id embedded in R2 URLs; `.5` now emits only opaque public media routes. Publish, public payload/media/share-page inspection, legacy private-URL denial, and Unpublish revocation pass in deployed QA; the live consent was then verified unchecked and the separate production Publish modal was opened and closed with `Keep Private` without publishing the fighter.
