...
- **The full production app and durable backend are live**: `https://insertplayer.ai` serves the complete product shell with Clerk Production and dedicated live Stripe configuration. Production Worker `0.17.0`, version `6fe7e56a-dea5-4989-983b-5aa819ac3e5a`, runs the `FIGHTER_GENERATION` Workflow plus EU `IMAGE_PROCESSOR` Container at `https://api.insertplayer.ai`; Pages deployment `580c95f2-8420-4da5-98e3-b5594f825ff4` serves source commit `9755b10`. The 253-test production gate, public Worker smoke, frontend live build/CSP smoke, and full live-readiness checks pass. Manual Wrangler OAuth promotion works, but the GitHub production secret currently fails its first D1 call with Cloudflare `7403` for the configured account; replace it with a long-lived token scoped to the exact SqaaS account with D1, Worker Scripts, R2, Pages, Containers, and Workflow deployment access.
- **Cloudflare QA is physically isolated and publicly testable**: sandbox Worker `0.17.0`, version `305c5499-c352-4664-aa55-9e0fda79ef1e`, runs at `https://insert-player-api-sandbox.shellbot.workers.dev` with migrations through `0019`, a separate `FIGHTER_GENERATION` Workflow and `IMAGE_PROCESSOR` Container, Clerk/Stripe/provider/anonymization secrets, EU D1/R2, and separate provider reserves. Sandbox Worker smoke and Pages deployment `b2b66dbd` pass with legal version `2026-08-23.1`; the Cancellations page and unchecked generation consent were inspected in the deployed UI. A signed-in real Victory animation Retry and Upright source Retry each survived navigation, reconnected in Gallery, committed exactly one credit, preserved prior versions, and completed with clean playable assets.
