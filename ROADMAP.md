...
- **Insert Player has isolated production storage**: D1 `insert-player-db` (`aa9e6ba6-8a7f-4261-ac31-986e6cf44659`) and R2 `insert-player-assets` are in the EU. Production has migrations through `0019`, including durable generation/retry jobs and the provider-request cache. Provider spend has atomic monthly and rolling reserves plus a permanent per-call cost ledger keyed by billing operation/outcome; scheduled cleanup cannot delete that ledger, credits, fighters, versions, users, tombstones, or R2 assets. R2 remains private behind the Worker and its `temp/` lifecycle deletes after one day.
- **The full production app and durable backend are live**: `https://insertplayer.ai` serves the complete product shell with Clerk Production and dedicated live Stripe configuration. Production Worker `0.17.0`, version `b6d1e2c2-bd10-4a1a-a924-b891661bd5ee`, runs the `FIGHTER_GENERATION` Workflow plus EU `IMAGE_PROCESSOR` Container at `https://api.insertplayer.ai`. Pages deployment `7f94c5a3-19e7-4a31-bd36-0a69c8bd7dfd` serves source commit `7dbb565`; its live build/CSP and canonical-route smoke passed after exact cache purge. The last complete protected production Action remains `32691593066`. The temporary GitHub Cloudflare OAuth token has since expired with `9109 Invalid access token`, so newer production/development Actions stop before remote migration; replace it with a durable account token scoped to D1, Worker Scripts, R2, Pages, Containers, Workflows, and Cache Purge.
- **Cloudflare QA is physically isolated and publicly testable**: sandbox Worker `0.17.0`, version `44d04769-c49d-4beb-be70-65925d1081d6`, runs at `https://insert-player-api-sandbox.shellbot.workers.dev` with migrations through `0019`, a separate `FIGHTER_GENERATION` Workflow and `IMAGE_PROCESSOR` Container, Clerk/Stripe/provider/anonymization secrets, EU D1/R2, and separate provider reserves. Sandbox Worker smoke and Pages deployment `ddb23525-bccd-4b70-b3fe-c90602d2c569` serve develop commit `074383e` and pass with legal version `2026-08-23.1`; the Cancellations page and unchecked generation consent were inspected in the deployed UI. A signed-in real Victory animation Retry and Upright source Retry each survived navigation, reconnected in Gallery, committed exactly one credit, preserved prior versions, and completed with clean playable assets.
...
- **Local hardening is green**: `npm run check:production` passes 283 tests across 57 files, the provider benchmark, Worker types, a clean migration replay through `0019`, the isolated prelaunch scan, durable-job concurrency/recovery, and all security/economic guards. Real Miniflare D1/R2 tests cover identical job-start races, upload races, progressive sync, pre-provider API release, first-provider charge commit, failed-spend retention, and concurrent release races without cost residue, plus billing reconciliation, moderation, provider budgets, private publication payloads, retention, tier profitability, exact Arcade-source restoration, restart safety, and bounded multipart provider-response caching. Node is pinned to 22.23.2 and Wrangler `4.125.0` generated the checked Worker types.
- **Final beta evidence**: replace the expired GitHub Cloudflare OAuth token with a durable account token that also grants Cache Purge; finish and activate the QA-approved Gemini-only Arcade roster; prove receipt/reply for both support aliases; use two fresh Clerk Production Google users for the authenticated launch smoke; complete one live Stripe payment/webhook smoke; run real-phone QA and physical two-device sign-in/import/play validation. Production Turnstile success and exact-token replay rejection are complete. Clerk Production, the full Pages app, durable Workflow/Container backend, dedicated Google OAuth, and isolated Live Stripe configuration are active. Apple is not a beta launch dependency and remains disabled until dedicated credentials can be created and tested.
