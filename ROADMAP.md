...
- **The full production app and durable backend are live**: `https://insertplayer.ai` serves the complete product shell with Clerk Production and dedicated live Stripe configuration. Production Worker `0.17.0`, version `b6d1e2c2-bd10-4a1a-a924-b891661bd5ee`, runs the `FIGHTER_GENERATION` Workflow plus EU `IMAGE_PROCESSOR` Container at `https://api.insertplayer.ai`; Pages deployment `3e9e05c4-46ca-4d2e-8115-d69cb54fd7a7` serves source commit `9dd468d`. The 282-test production gate, public Worker smoke, frontend live build/CSP smoke, and full live-readiness checks pass from protected GitHub Action `32691593066`. The current temporary OAuth credential completed D1 migrations plus Worker/Container/Pages deployment, but exact zone-cache purge returned `401`; replace it with a long-lived token scoped to the exact SqaaS account with D1, Worker Scripts, R2, Pages, Containers, Workflows, and Cache Purge access.
