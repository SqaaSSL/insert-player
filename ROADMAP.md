...
- **Cloud pull is content-incremental in real QA**: the browser importer compares version/content hashes and unique animation names, downloads only missing processed/RAW blobs, and preserves local metadata for retryable optional assets. A no-op Gallery reload of the 48-version QA fighter reached `Ready` in `1.945s` instead of spending roughly 30 seconds re-downloading known blobs, with no new uploads or versions.
...
- **Stripe QA is isolated and validated through the Checkout boundary**: the v2 sandbox catalog is active at 11/20/47 credits for €14.99/€24.99/€56.99; stale v1 prices are inactive but purchase history is preserved. A signed-in Starter Checkout completed at €14.99, returned to `/menu`, and moved the wallet from 6 to 17. Remote D1 records one `paid` 11-credit Checkout and one `+11` ledger entry alongside the untouched historical `paid` 6-credit purchase. The wildcard webhook remains sandbox-only. Live keys/catalog/webhook and a live purchase remain outstanding.
...
- **Cloud sync/import exists on the frontend** in `src/services/CloudFighters.ts`. Create flow auto-syncs after generation when signed in; Gallery has a manual Sync Cloud action; Gallery and Roster refresh owned cloud fighters into IndexedDB so another device can become playable after sign-in and later receive remote retries/upgrades for fighters it already imported. Imports are playable-first: owned fighter lists stay light, missing or newer fighters fetch `/api/fighters/:id` for full `spriteVersions`, optional source/raw assets may be skipped with debug warnings, current sprite sheets import per animation, and one bad cloud fighter no longer blocks the rest of the roster. Imported fighter metadata uses the Worker/D1 updated timestamp when available so cross-device refresh decisions do not depend on the importing browser's local clock. The runtime sprite loader now fallback-fills missing animation rows from mapped fallbacks, idle, or the first available animation so partially imported private fighters do not render invisible move states. Gallery rename/delete also patches/deletes the cloud fighter when a synced `cloudFighterId` is present.
