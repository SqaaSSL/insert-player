...
- **The full production app and durable backend are live**: `https://insertplayer.ai` serves the complete product shell with Clerk Production and dedicated live Stripe configuration. Production Worker `0.17.0`, version `62e6477d-9bc0-4d19-a7c0-2da8b26c48ac`, runs the `FIGHTER_GENERATION` Workflow plus EU `IMAGE_PROCESSOR` Container at `https://api.insertplayer.ai`; Pages deployment `740a11e2` serves the matching frontend. The 199-test production gate, public Worker smoke, frontend live build/CSP smoke, and full live-readiness checks passed after the no-restoration release was promoted. The production Action is structurally complete, but its account token still needs Cloudflare `Containers: Write`; this promotion used the authorized Wrangler OAuth session after the Action correctly applied migrations and then failed closed on that missing token permission.
